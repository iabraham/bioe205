...
begin
	local sys = tf([1/3, 1], [1])
	p4 = bodeplot(sys, label=false,layout=(1,2), size=(650,300), plot_title="Type 2", xlabel="Frequency [rad/s]", margins=3mm, yminorticks=true)
	vline!(p4[2], [3], linestyle=:dash, label=L"\omega=3", xminorticks=true)
	hline!(p4[2], [45], linestyle=:dash, label=L"45^\circ", ylim=(-200,100))
	p4 = vline!(p4[1], [3], linestyle=:dash, label=L"\omega=3", xminorticks=true,ylim=(0.001, 10))
end

# ╔═╡ bf420d69-8d27-4f40-bc60-b48a59aa547b
begin
	local sys = tf([1], [1/4, 1/2, 1])
	p5 = bodeplot(sys, label=false,layout=(1,2), size=(650,300), plot_title="Type 3", xlabel="Frequency [rad/s]", margins=3mm)
	vline!(p5[2], [2], linestyle=:dash, label=L"\omega_n=2", xminorticks=true)
	hline!(p5[2], [-90], linestyle=:dash, label=L"\phi=-\pi/2", xminorticks=true, ylim=(-200,100))
	vline!(p5[1], [2], linestyle=:dash, label=L"\omega_n=2", xminorticks=true, yminorticks=true, ylim=(0.001, 10))
end


# ╔═╡ 0fd6b02d-9f1f-4af0-96e1-b2eaa370ed06
savefig(p4, joinpath(@__DIR__, "output/type2_ex1"))

# ╔═╡ dd88e450-700a-490a-bc80-e012afdfb944
savefig(p5, joinpath(@__DIR__, "output/type3_ex1"))

# ╔═╡ 1677dd43-a048-4475-aa2e-3e3a02addd97
begin
	sys = tf([1, 3], [1, 2, 4, 0])
	p6 = bodeplot(sys,label=false,layout=(1,2), size=(650,350), plot_title="Combined Bode plot", xlabel="Frequency [rad/s]", margins=3mm, top_margin=5mm)
	mag = vline!(p6[1],[2,3], label=L"(\omega_n, 1/\tau) =2,3", xminorticks=true);
	pha = vline!(p6[2],[2,3], label=L"(\omega_n, 1/\tau) =2,3", xminorticks=true, title="\nPhase");
	hline!(p6[1], [3/4], linestyle=:dash, label=L"\log(3/4)", title="\nMagnitude", yminorticks=true)
	# hline!(pp[2], [-145], linestyle=:dash, label=L"-145^\circ", yminorticks=true)
	# plot(mag, pha, layout=(1, 2))
end

# ╔═╡ d1d2aada-aef4-4bb8-9ec9-ea834c05d215
savefig(p6, joinpath(@__DIR__, "output/ex1_final"))
