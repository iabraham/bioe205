...
# ╔═╡ 986168e4-b528-11ed-3827-b394ba70e7e7
# ╠═╡ show_logs = false
using DSP ,Plots, PlutoUI, NumericalIntegration, Waveforms, FFTW

# ╔═╡ 00845556-7512-44f1-b95c-c1a96b7b3aa0
begin
	box(t) = abs(t) < 1 ? 1 : 0 
	shifts = -π:0.05:π
	vals = Array{Float64}(undef, length(shifts))
end

# ╔═╡ 8ef7a622-c8d8-4c4d-8fad-5a88f326ee36
dd = @animate for (i, ϕ) in enumerate(shifts)
	t = -π:0.001:π
	f = box.(t)
	g = box.(t.-ϕ)
	prod = f .* g
	vals[i] = integrate(t, prod) / 2
	pp = plot(t, box.(t), label=false)
	pp = plot!(pp, t, box.(t.-ϕ), label=false, xlabel="Time / lag axis")
	pp = plot!(pp, shifts[1:i], vals[1:i],title="Convolution of rectangular pulses", label="★ integral")
	# plot!(pp,t, h)
end

# ╔═╡ b28db7a6-f8a8-47c7-bc78-1fe1f09d7b9e
gif(dd,joinpath(@__DIR__,"output/pulse-conv-anim.gif"))

# ╔═╡ a9083095-f58a-4c9b-ade1-52f18513c1ae
begin
	local t = 0:(1/160):1
	...
	ya(t) = conv(x.(t), h.(t))
	p3 = plot(t,ya(t)[1:length(t)], title="Output via convolution", label=false)
...
# ╔═╡ e987a563-9a74-4d35-8d53-55e0c7a18c45
begin
	N, t = 10, 0:0.01:2π
	f(t) = sin(t) + rand()
	plot(t, f.(t), label="Noisy sine")
	z(t) = conv(f.(t), repeat([1/N],N))
	plot!(LinRange(0, 2π, length(z(t))), z(t), label="Smoothened sine", 
		title="Taking a 10 point average")
end

# ╔═╡ 9fbf516e-9c84-4422-b652-aed377c4e0e1
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "8d6fbc53d2794b04ef5c1f51b41de2853f67fa84"
