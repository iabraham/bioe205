...
begin
	p1 = plot(w, mag[:], xscale=:log10, label=false, ylabel="|TF(ω)|", title="\nMagnitude spectrum")
	p2 = plot(w, phase[:],xscale=:log10, label=false, ylabel="arg TF(ω)", title="\nPhase spectrum")
	sys_spec = plot(p1, p2, layout=(1, 2), xlabel="Frequency (rad/s)", plot_title="System Spectrum", size=(800, 400), margins=5mm)
end
